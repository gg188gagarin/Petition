...
<?php } ?>
    ...
    <div class="form-group">
        ...
        <?php if (! empty($petition)) { ?>
            <textarea class="form-control input_color" id="description" name="description" rows="3"><?= $petition['description'] ?>
            </textarea>
        <?php } elseif (! empty($_REQUEST['description'])) { ?>
            <textarea class="form-control input_color" id="description" name="description" rows="3"><?= $_REQUEST['description'] ?>
            </textarea>
        <?php } else { ?>
            <textarea class="form-control input_color" id="description" name="description" rows="3"></textarea>
        <?php } ?>
    </div>
    <div class="form-group" style="padding-top: 1em">
        <?php if ((! empty($petition)) || (! empty($_REQUEST['name']))) { ?>
            <button class="btn " style="background-color: #8880df" type="submit">Update</button>
        <?php } else { ?>
            <button class="btn " style="background-color: #8880df" type="submit" >Create</button>
        <?php } ?>
    </div>
    <br>
    <?php if (!empty($errors)) { ?>
